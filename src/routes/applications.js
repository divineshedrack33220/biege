...
// POST /api/applications/cleanup - Remove duplicate applications (by email)
router.post('/cleanup', auth, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            totalBefore: allApps.length,
            duplicatesRemoved: duplicates.length,
            remaining: kept.length,
            message: `Removed ${duplicates.length} duplicate application(s). ${kept.length} unique applications remain.`
        });
    } catch (error) {
