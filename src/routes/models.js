...
router.get(
  ...
  [
    ...
    query('modelSize')
      ...
      .isString()
      .matches(/^[\w\s-]+$/)
      .withMessage('Model size must be a valid string (letters, numbers, spaces, or hyphens)'),
  ],
  async (req, res) => {
    ...
    try {
      const { category, name, modelSize, page = 1, limit = 6 } = matchedData(req);
      const query = {};
      if (category && category !== 'all') query.category = category;
      if (name) query.name = { $regex: name, $options: 'i' };
      if (modelSize) query.modelSize = { $regex: modelSize, $options: 'i' }; // Flexible matching for modelSize
...
router.post(
  ...
  [
    ...
    body('modelSize')
      ...
      .trim()
      .isLength({ max: 100 })
      .withMessage('Model Size cannot exceed 100 characters'),
    body('description')
      .optional()
      .trim()
      .isLength({ max: 1000 })
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { name, category, description, height, bust, waist, hips, hair, eyes, shoes, modelSize, placements, socialLinks } = data;
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const data = matchedData(req);
      const { name, category, description, height, bust, waist, hips, hair, eyes, shoes, modelSize, placements, socialLinks, imagePublicId } = data;
